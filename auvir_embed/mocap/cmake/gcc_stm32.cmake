...
IF(NOT TOOLCHAIN_PREFIX)
     MESSAGE(WARNING " No TOOLCHAIN_PREFIX specified")
ELSE()
     MESSAGE(STATUS "TOOLCHAIN_PREFIX specified: " ${TOOLCHAIN_PREFIX})
ENDIF()
...
# workaround. TODO: find compiler using path
IF(WIN32)
SET(EXE_ENDING ".exe")
ENDIF(WIN32)
IF(UNIX)
...
SET(COMPILER_C_NAME "gcc")
SET(COMPILER_CXX_NAME "g++")

find_program(COMPILER_C NAMES ${TARGET_TRIPLET}-${COMPILER_C_NAME}${EXE_ENDING} PATHS ${TOOLCHAIN_BIN_DIR} NO_DEFAULT_PATH)
find_program(COMPILER_CXX NAMES ${TARGET_TRIPLET}-${COMPILER_CXX_NAME}${EXE_ENDING} PATHS ${TOOLCHAIN_BIN_DIR} NO_DEFAULT_PATH)

MESSAGE(STATUS "Which compiler c: " ${COMPILER_C})
MESSAGE(STATUS "Which compiler c++: " ${COMPILER_CXX})
MESSAGE(STATUS "TOOLCHAIN_BIN_DIR: " ${TOOLCHAIN_BIN_DIR})

SET(CMAKE_C_COMPILER ${COMPILER_C})
SET(CMAKE_CXX_COMPILER ${COMPILER_CXX})
#CMAKE_FORCE_C_COMPILER(${TOOLCHAIN_BIN_DIR}/${TARGET_TRIPLET}-${COMPILER_C_NAME}${EXE_ENDING} GNU)
#CMAKE_FORCE_CXX_COMPILER(${TOOLCHAIN_BIN_DIR}/${TARGET_TRIPLET}-${COMPILER_CXX_NAME}${EXE_ENDING} GNU)
#original: SET(CMAKE_ASM_COMPILER ${TOOLCHAIN_BIN_DIR}/${TARGET_TRIPLET}-gcc)
SET(CMAKE_ASM_COMPILER ${COMPILER_C})
...
SET(CMAKE_OBJCOPY ${OBJCOPY_PRG} CACHE INTERNAL "objcopy tool")
SET(CMAKE_OBJDUMP ${OBJDUMP_PRG} CACHE INTERNAL "objdump tool")
...
SET(CMAKE_C_FLAGS_DEBUG "-Og -g3 -O0" CACHE INTERNAL "c compiler flags debug")
SET(CMAKE_CXX_FLAGS_DEBUG "-Og -g3 -O0" CACHE INTERNAL "cxx compiler flags debug")
...
FUNCTION(STM32_ADD_HEX_BIN_TARGETS TARGET)
    ...
    ELSE()
      SET(FILENAME "${TARGET}")
    ENDIF()
    message(STATUS "info; TARGET=${TARGET} COMMAND=${CMAKE_OBJCOPY} FILENAME=${FILENAME}")
...
FUNCTION(STM32_SET_TARGET_PROPERTIES TARGET)
    IF(NOT STM32_CHIP_TYPE)
        ...
        ENDIF()
    ENDIF()
    MESSAGE(STATUS "DEBUG message for STM32_SET_TARGET_PROPERTIES: TARGET: ${TARGET}; CHIP_TYPE: ${STM32_CHIP_TYPE}")
