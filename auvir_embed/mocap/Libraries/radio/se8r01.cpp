...
///
/// on initialization se8r01 check
/// http://forum.easyelectronics.ru/viewtopic.php?f=9&t=21484
///
...
NRF_Module default_module =
    NRF_Module(GPIO_PIN(NRF24_CE1_Port, NRF24_CE1_Pin),
               GPIO_PIN(NRF24_CSN1_Port, NRF24_CSN1_Pin),
               GPIO_PIN(NRF24_IRQ1_Port, NRF24_IRQ1_Pin));
NRF_Module data_module = NRF_Module(GPIO_PIN(NRF24_CE2_Port, NRF24_CE2_Pin),
                                    GPIO_PIN(NRF24_CSN2_Port, NRF24_CSN2_Pin),
                                    GPIO_PIN(NRF24_IRQ2_Port, NRF24_IRQ2_Pin));
...
static void set_rx_tx_mode(NRF_Module *radiomodule, NRF24_Mode mode);
...
static uint8_t SPI_Read_Buf(NRF_Module *radiomodule, uint8_t reg, uint8_t *pBuf,
                            uint8_t bytes);
static uint8_t SPI_Write_Buf(NRF_Module *radiomodule, uint8_t reg,
...
TransmissionStatus nrf24_last_messageStatus(NRF_Module *radiomodule) {
  ...
  if (rv & iSTATUS_TX_DS) {
    return TransmissionStatus::NRF24_TRANSMISSON_OK;
  }
  // Maximum retransmission count is reached //
  // Last message probably went missing ... //
  if (rv & iSTATUS_MAX_RT) {
    return TransmissionStatus::NRF24_MESSAGE_LOST;
  }
  // Probably still sending ... //
  else {
    return TransmissionStatus::NRF24_MESSAGE_SENDING;
  }
}
...
void nrf24_reset(NRF_Module *radiomodule) {
  ...
  nrf24_write_register(radiomodule, STATUS,
...
void setup_radio(NRF_Module *radiomodule, NRF24_Mode mode) {
  ...
  // set EN_AA, EN_RXADDR, RF_CH (needless?), RF_SETUP (needless?), AW,
  // SETUP_RETR, TX_ADDR, RX_ADDR_P*
...
void radio_update_settings(NRF_Module *radiomodule, RadioDevInfo *devinfo) {
  ...
  nrf24_write_register_buf(radiomodule, iRF_BANK0_TX_ADDR,
                           devinfo->address_usbdevice.byte_array, TX_ADR_WIDTH);

  // Receive address
  nrf24_write_register_buf(radiomodule, iRF_BANK0_RX_ADDR_P0,
                           devinfo->address.byte_array, TX_ADR_WIDTH);
...
void nrf_receive_handler(NRF_Module *radiomodule) {
  ...
  if (status & (1 << RX_DR)) {
    ...
    SPI_Read_Buf(radiomodule, R_RX_PAYLOAD, (uint8_t *)&rx_message,
                 TX_PLOAD_WIDTH);
...
void TXX(NRF_Module *radiomodule) {
  ...
  // | Bit 0   |
  // Reserved | MASK_RX_DR | MASK_TX_DS | MASK_MAX_RT | EN_CRC | CRCO  | PWR_UP
  // | PRIM_RX |
  // By setting one of the MASK bits high, the corresponding IRQ source is
  // disabled. By default all IRQ sources are enabled.
  // nrf24_write_register(radiomodule,iRF_BANK0_CONFIG, (1 << MASK_RX_DR) | (0
  // << MASK_TX_DS) | (0 << MASK_MAX_RT) | (1 << EN_CRC) | (1 << CRCO)  | (1 <<
  // PWR_UP) | (0 << PRIM_RX) );
  // delay_us(200);
  ...
  SPI_Write_Buf(radiomodule, iRF_CMD_WR_TX_PLOAD, (uint8_t *)&tx_message,
                TX_PLOAD_WIDTH);
...
// HAL_Delay(2);
  ...
  uint8_t packet_not_sent = ((!SPI_Read(radiomodule, iRF_BANK0_STATUS)) &
                             (iSTATUS_TX_DS | iSTATUS_MAX_RT));
  uint8_t elapsed = HAL_GetTick() - tickstart;

  while (packet_not_sent && elapsed < Delay) {
    packet_not_sent = ((!SPI_Read(radiomodule, iRF_BANK0_STATUS)) &
                       (iSTATUS_TX_DS | iSTATUS_MAX_RT));
  ...
  if (tx_status == TransmissionStatus::NRF24_TRANSMISSON_OK) {
    HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13);
    // HAL_GPIO_TogglePin(GPIO_LED_PORT,GPIO_LED_PIN);
  } else if (tx_status == TransmissionStatus::NRF24_MESSAGE_LOST) {
    // HAL_GPIO_TogglePin(GPIOC,GPIO_PIN_13);
    // HAL_GPIO_WritePin(GPIOC,GPIO_PIN_13, GPIO_PIN_RESET);//nop
  } else {
  }

  nrf24_write_register(radiomodule, iRF_CMD_WRITE_REG + iRF_BANK0_STATUS,
                       0xff); // clear RX_DR or TX_DS or MAX_RT interrupt flag
}

static void radio_settings(NRF_Module *radiomodule) {
  // Enable Auto Acknowledgment
  nrf24_write_register(radiomodule, iRF_BANK0_EN_AA,
                       (1 << ENAA_P0) | (0 << ENAA_P1) | (0 << ENAA_P2) |
                           (0 << ENAA_P3) | (0 << ENAA_P4) | (0 << ENAA_P5));

  // Enable RX addresses (pipes)
  nrf24_write_register(radiomodule, iRF_BANK0_EN_RXADDR,
                       (1 << ERX_P0) | (0 << ERX_P1) | (0 << ERX_P2) |
                           (0 << ERX_P3) | (0 << ERX_P4) | (0 << ERX_P5));

  // SETUP_AW number of bytes for address
  // 4 byte adress, but use 5 byte address! TODO: research
  // http://forum.easyelectronics.ru/viewtopic.php?f=9&t=21484
  // 11 5 bytes
  // 10 4 bytes
  // 01 Illegal
  ...
  nrf24_write_register(radiomodule, iRF_BANK0_SETUP_RETR,
  ...
  nrf24_write_register(radiomodule, iRF_BANK0_RF_SETUP,
                       (0 << CONT_WAVE) | (1 << PA_PWR_3) | (0 << RF_DR_LO) |
                           (1 << RF_DR_HIG) | (0x7 << PA_PWR));
  // original comment: 2mps 0x4f, which is 1001111 TODO: 2mps is
  // (RF_DR_LO,RF_DR_HIG) = (0,1) according to datasheet, and 0x4f stands for
  // 1mps wtf?
  // 0x47 1000111
  ...
  nrf24_write_register_buf(radiomodule, iRF_BANK0_TX_ADDR, TX_ADDRESS,
                           TX_ADR_WIDTH);

  // Use the same address on the RX device as the TX device
  nrf24_write_register_buf(radiomodule, iRF_BANK0_RX_ADDR_P0, TX_ADDRESS,
                           TX_ADR_WIDTH);
...
static void se8r01_powerup(NRF_Module *radiomodule) {
  ...
  // Reserved | MASK_RX_DR | MASK_TX_DS | MASK_MAX_RT | EN_CRC | CRCO  | PWR_UP
  // | PRIM_RX |
  nrf24_write_register(radiomodule, iRF_BANK0_CONFIG,
                       (0 << MASK_RX_DR) | (0 << MASK_TX_DS) |
                           (0 << MASK_MAX_RT) | (1 << EN_CRC) | (0 << CRCO) |
                           (1 << PWR_UP) | (1 << PRIM_RX));
  ...
  nrf24_write_register(radiomodule, iRF_BANK0_PRE_GURD,
                       0x77); // 2450 calibration
}
...
static void set_power(NRF_Module *radiomodule, NRF_PowerState power_state) {
  ...
  if (power_state == NRF_PowerState::PowerOFF) {
    config_reg &= (~(1 << PWR_UP)); //(~(1 << PWR_UP)) -  PWR_UP bit to 0 and
                                    // all other bits to 1
  } else // if (power_state == NRF_PowerState::PowerOFF)
  {
    config_reg |=
        (1
         << PWR_UP); //(1 << PWR_UP) -  PWR_UP bit to 1 and all other bits to 0
  }
...
static void power_on_rx(NRF_Module *radiomodule) {
  ...
  // | PRIM_RX |
  nrf24_write_register(radiomodule, iRF_BANK0_CONFIG,
                       (0 << MASK_RX_DR) | (1 << MASK_TX_DS) |
                           (1 << MASK_MAX_RT) | (1 << EN_CRC) | (0 << CRCO) |
                           (1 << PWR_UP) | (1 << PRIM_RX));
  // start listening
  delay_us(10);
  nrf24_ce_set(radiomodule, GPIO_PIN_SET);
