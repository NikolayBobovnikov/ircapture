...
// for single person on motion platform, sensor will receive angles from all
// beamers.
// In this case all registered beamers are nearby
// But for multi person with arena platform, sensor won't be able to receive
// angles from all beamers,
// Instead, each sensor should receive data from subset of beamers (e.g. which
// are nearby)
...
typedef struct RadioAddress { uint8_t byte_array[TX_ADR_WIDTH]; } RadioAddress;

typedef struct RadioMessage {
  uint8_t header;
  uint8_t data[31];
  // Maximum size of radiomessage for radiomodule is 32 bytes, so upper bound
  // for data is 31 bytes
...
typedef struct RadioDevInfo {
  ...
  uint16_t prev_id; // TODO: workaround this. Need extra space and processing
                    // for prev_id
...
// TODO: make sure size of SensorData is the same as size of radio buffer (rx/tx
// payload) if it is not dynamic
#define MAX_SENSOR_DATA_SIZE                                                   \
  27 // maximum size for data member to be able to contain all types of data
     // (BeamerData, IMUData)
...
typedef enum RM_Dest_e { Dest_UsbDevice, Dest_Sensor, Dest_Beamer } RM_Dest_e;
...
void radio_tx_set_message_header(RM_WhoAmI_e whoami, RM_Dest_e dst,
                                 RM_Typ_e typ);
