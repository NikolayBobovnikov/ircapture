...
enum class GPIO_STATE { LOW = GPIO_PIN_RESET, HIGH = GPIO_PIN_SET };
...
public:
  ...
  // TODO: read on rule of 3/5
  GPIO_PIN(const GPIO_PIN &other) {
    _Pin = other.Pin();
    _Port = other.Port();
  }
  GPIO_PIN &operator=(GPIO_PIN &&) = default;

  void Set(GPIO_STATE state) const {
    HAL_GPIO_WritePin(_Port, _Pin, static_cast<GPIO_PinState>(state));
  }
