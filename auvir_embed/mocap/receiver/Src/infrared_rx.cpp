...
ReceiverStates ReceiverState = RX_WAITING_FOR_START_BIT;
...
#define DECODE_BIT_IN_WORD(word)                                               \
  (word |= 1 << (rx_total_bits - rx_current_bit_pos - 1))
...
void setup_ic_timer() {
  ...
  HAL_TIM_IC_ConfigChannel(&htim4, &sConfigIC,
                           TIM_CHANNEL_2); // TIM_CHANNEL_2? TODO
...
static inline void receive_handler() {
   ...
   *                      |              |    |    |    |dataword|between |
   * |        |        |    |    |    |             |
   *                      |     750      |350 |350 |350 |  500   |words   |
   * |        |  500   |350 |350 |350 |    750      |
   *  ____________________|              |____|    |____|        |__500___|
  ...
  case RX_WAITING_FOR_START_BIT: {
    ...
    {
      if (is_correct_timming_interframe_delay()) // TODO: check delay before
                                                 // data frame
      {
        debug_interframe_delay();

        ptim_data_read->Instance->ARR =
            PreambleTotalLength; // will update on the "rising edge" of first
                                 // data bit
  ...
  case RX_START_BIT_PROCESSING: {
    // Start sequence consists of signal sequence {1,0,1,0}, each bit of length
    // PeriodOfStartStopBits
    switch (StartStopSequenceReceiveState) {
    case Rx_PREAMBLE_BIT_1: {
      if (is_1_to_0_edge()) // falling edge -> bit finished
      {
        if (is_correct_timming_preamble_bit()) // verify that it is within time
                                               // interval boundaries
        {
      ...
      // 1) probing timer is not used here 2) should not be dataread timer event
      // (it is either stopped or waiting for when data should begin)
      // Thus, need reset if event is different than what has been processed
      // above
    ...
    case Rx_PREAMBLE_DELAY_1: {
      ...
      {
        if (is_correct_timming_preamble_delay()) // verify that it is within
    ...
    case Rx_PREAMBLE_BIT_2: {
      if (is_1_to_0_edge()) // falling edge -> bit finished
      {
                                               ...
                                               // interval boundaries
        {
          ...
          // After second preamble bit there is last preamble delay, after which
          // data will begin
          // Thus, need to start dataread timer here (using last edge as sync
          // event)
    ...
    case Rx_PREAMBLE_DELAY_2: {
      ...
      // this time it should be update event for dataread timer started on
      // previous step
      if (_is_uptimer_update_event) // delay finished
      {
        ...
        // At this point we are expected to be at the very beginning of the data
        // frame, so
        // to start reading data bits need to wait exactly HalfDataBitLength
        // ticks
        // But we are out of sync with offset of DataBitLength, for some reason
        ptim_data_read->Instance->ARR =
            HalfDataBitLength; // DataBitLength + DataBitLength;
      ...
      // should not reset if not update event, since on the edge of preamble
      // delay and data bit (may be rising edge)
      break;
    }
    default: {
      reset_receiver_state();
  ...
  case RX_DATA_PROCESSNG: {
    if (_is_uptimer_update_event) {
      ...
      case DATAFRAME_1_BEAMER_ID: {
        if (0 == rx_current_bit_pos) // execute only once, when first bit is
                                     // being processed
        {
          // change period only when the very first bit of the whole data frame
          // is being processed
      ...
      case DATAFRAME_3_ANGLE_REV: {
        // calculate total bit number only once, when first bit is being
        // processed
  ...
  case RX_STOP_BIT_PROCESSING: {
    ...
    case Rx_PREAMBLE_START: {
      ...
      // should not reset if not update event, since on the edge of data bit and
      // preamble delay  (may be falling edge)
      break;
    }
    case Rx_PREAMBLE_DELAY_1: {
      if (is_0_to_1_edge()) // rising edge -> delay finished, bit started
      {
        if (is_correct_timming_preamble_delay()) // check delay length in
                                                 // allowed interval
        {
          StartStopSequenceReceiveState = Rx_PREAMBLE_BIT_1;
          break;
      ...
      // currently, we are in the edge of last data bit and preamble/epilogue
      // delay.
      // Since data bit may be 1, it may be falling edge near data bit end,
      // instead of rising edge near preamble/epilogue bit start
    ...
    case Rx_PREAMBLE_BIT_1: {
      if (is_1_to_0_edge()) // falling edge -> bit finished
      {
        if (is_correct_timming_preamble_bit()) // check bit length in allowed
                                               // interval
        {
...
static inline void process_received_data() {
  /// we successfully received data, send corresponding event for listeners to
  /// read from the data buffer
  /// verify correctness
  data_frame_delta =
      rx_data_frame._2_angle_code ^ (~rx_data_frame._3_angle_code_rev);
...
static inline void get_logical_level() {
  ...
  } else if (HAL_GPIO_ReadPin(IR_IN_Gpio_Port, IR_IN_Gpio_Pin) ==
             GPIO_PIN_RESET) {
...
static inline void send_dataready_signal() {
  ...
  // Each sensor checks its ID. If it is first in the array (ID == 1), it first
  // sends message with its ID and data, without waiting
  // Hub and all remaining sensors are waiting for receiving the message
  // Each sensor checks if it is his turn to send data (its Id is the next ID
  // after that which has been received).
  ...
  // So the first sensor (with ID == 1) checks if ID_msg == N, if so - it is
  // next to send data
