...
extern TIM_HandleTypeDef *const phtim_envelop = &htim3;
extern TIM_HandleTypeDef *const phtim_pwm = &htim2;
extern TIM_HandleTypeDef *const phtim_delay = &htim4;
}
...
void init_data() {
  ...
  if (TX_WAITING == TransmitterState) {
    ...
    // data is still being transmitted. Need to finish previous transmission
...
void transmit_handler() {
  ...
  case TX_PREAMBLE: {
    ...
    case Tx_PREAMBLE_BIT_1: {
      ...
      auvir::phtim_envelop->Instance->ARR = PreambleBitCorrected;
      StartStopSequenceTransmitState = Tx_PREAMBLE_DELAY_1;
      break;
    }
    // First short delay
    case Tx_PREAMBLE_DELAY_1: {
      force_envelop_timer_output_off(); // TODO done anyway in timer interrupt
                                        // handler?
      auvir::phtim_envelop->Instance->ARR = PreambleDelayCorrected;
  ...
  case TX_DATA: {
    auvir::phtim_envelop->Instance->ARR = DataBitLength;

    switch (TxDataFrameState) {
    case (DATAFRAME_1_BEAMER_ID): {
      tx_total_bits = sizeof(tx_data_frame._1_beamer_id) * 8;
      tx_bit = (tx_data_frame._1_beamer_id >>
                (tx_total_bits - tx_current_bit_pos - 1)) &
               1;
      p_w_modulate(tx_bit);
      // go to next bit.
    ...
    case (DATAFRAME_2_ANGLE): {
      ...
      tx_bit = (tx_data_frame._2_angle_code >>
                (tx_total_bits - tx_current_bit_pos - 1)) &
               1;
      p_w_modulate(tx_bit);
      // go to next bit.
    ...
    case (DATAFRAME_3_ANGLE_REV): {
      ...
      tx_bit = (tx_data_frame._3_angle_code_rev >>
                (tx_total_bits - tx_current_bit_pos - 1)) &
               1;
      p_w_modulate(tx_bit);
      // go to next bit.
      if (tx_current_bit_pos < tx_total_bits) {
        ...
        force_envelop_timer_output_off();
        auvir::phtim_envelop->Instance->ARR = EpilogueDelayCorrected;
  ...
  case TX_EPILOGUE: {
    ...
    case Tx_PREAMBLE_BIT_1: {
      force_envelop_timer_output_on();
      auvir::phtim_envelop->Instance->ARR = EpilogueBitCorrected;
      StartStopSequenceTransmitState = Tx_PREAMBLE_DELAY_1;
      break;
    }
    case Tx_PREAMBLE_DELAY_1: {
...
static inline void reset_transmitter() {
  ...
  auvir::phtim_envelop->Instance->ARR = InterframeDelayLength;
  StartStopSequenceTransmitState = Tx_PREAMBLE_BIT_1;
  TxDataFrameState = DATAFRAME_0_NODATA;
  beamer_current_ir_channel = beamer_data_pin;
}
...
static inline void turn_off_all_beamer_pins() { shiftreg_send_8bit_data(0); }
...
static inline void reset_previous_update_current_beamer_pin() {
  ...
  // shiftreg_send_16bit_data(0xFF ^
  // beamer_ir_channel_array[beamer_current_ir_channel]);
...
static inline void force_envelop_timer_output_on() {
  ...
  if (_is_direct_logic) {
    ...
    HAL_TIM_PWM_Stop(auvir::phtim_pwm, auvir::pwm_tim_channel);
  }
}
