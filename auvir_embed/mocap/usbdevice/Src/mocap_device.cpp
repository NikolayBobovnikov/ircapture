...
class USB_Message {
public:
  USB_Message() : msgType(USBMessageType::USB_None), data{0} {}
  USBMessageType msgType;
  uint8_t data[31];
};

USB_Message tx_usb_message;
USB_Message rx_usb_message;
...
// stuff below is used to convert uint8_t* data to specific format, depending on
// message type
...
/// Sensor/Beamer with corresponding ID is registered <=> ID of array's element
/// at (index==ID) != 0
/// Sensor/Beamer with corresponding ID is lost connection <=> ID of array's
/// element at (index==ID) == 0 for  index==ID <= LastBeamerID
/// All Sensors/Beamers with ID > LastBeamerID are not registered and thus has
/// ID == 0
/// Element at index 0 is special (can be used to check if list of registered
/// devices is empty (LastID==0))
...
// TODO: change beamer_ids to bitset (so to store 256 bits will need 32 bytes,
// instead of 256)
...
// TODO: change sensor_ids to bitset (so to store 256 bits will need 32 bytes,
// instead of 256)
...
void register_usb_device() {
  ...
  if (UsbHostDone) {
    //
    USB_Message msg;
    msg.msgType = USBMessageType::USB_DeviceRegistrationRequest;
    ...
    if (is_usb_configured()) {
      ...
      if (rx_usb_message.msgType ==
          USBMessageType::USB_DeviceRegistrationInfo) {
...
void process_sensor_data() {
  ...
  // Monitor sensors which lost connection (compare receiveid id with expected
  // id)
  // TODO
  ++expected_sensor_id;
...
void process_beamer_registration_request() {
  ...
  if (assigned_beamer_id > 0) {
    ...
    radio_tx_set_message_header(WhoAmI_UsbDevice, Dest_Beamer,
...
void process_sensor_registration_request() {
  ...
  if (assigned_sensor_id > 0) {
    // prepare radio message
    radio_tx_set_message_header(WhoAmI_UsbDevice, Dest_Sensor,
                                Typ_RespondRegistration);
...
// finish processing message if message type is "registration request"
// (beamer/sensor)
void send_assigned_radiodevinfo_back() { TXX(&default_module); }
...
void try_register_beamer() {
  ...
  if (last_beamer_id == 0) {
    ...
    // check if there are beamers that marked as lost connection, if so - reuse
    // one of them
    try_reassign_beamer();
    // check if succeeded
    if (assigned_sensor_id == 0) {
      // if there was no beamer which lost connection, and there is a place for
      // new beamer, register new beamer
...
bool try_reassign_beamer() {
  ...
  for (uint8_t index = 1; index <= last_beamer_id; ++index) {
    if (registered_beamers[index].id == 0) {
      // use its ID and address for new beamer (will send them to new beamer
      // afterwards)
      ...
      // no need to change address (both beamer and usbdevice) - just use
      // existent address from beamer/sensor which lost connection
...
void register_new_beamer_in_array() {
  ...
  memcpy(&(registered_beamers[assigned_beamer_id].address),
         &Sensor_Beamer_DefaultAddress, sizeof(RadioAddress));
  memcpy(&(registered_beamers[assigned_beamer_id].address_usbdevice),
         &UsbDeviceAddress, sizeof(RadioAddress));
}
...
void try_register_sensor() {
  ...
  if (last_sensor_id == 0) {
    ...
    // check if there are sensors that marked as lost connection, if so - reuse
    // one of them
    try_reassign_sensor();

    // check if succeeded
    if (assigned_sensor_id == 0) {
      // if there was no sensor which lost connection, and there is a place for
      // new sensor, register new sensor
...
void try_reassign_sensor() {
  ...
  for (uint8_t index = 1; index <= last_sensor_id; ++index) {
    if (registered_sensors[index].id == 0) {
      ...
      // no need to change address - just use existent address from
      // beamer/sensor which lost connection
      // TODO FIXME: verify that using existent address from the array works
      // stop registration procedure
      // TODO FIXME: verify that break
      return;
...
void register_new_sensor_in_array() {
  ...
  // use default address for now
  memcpy(&(registered_sensors[assigned_sensor_id].address),
         &Sensor_Beamer_DefaultAddress, sizeof(RadioAddress));
  memcpy(&(registered_sensors[assigned_sensor_id].address), &UsbDeviceAddress,
         sizeof(RadioAddress));
}
...
void update_lost_beamers_from_sensordata() {
  ...
  // no deep copy, according to
  // http://stackoverflow.com/questions/2302351/assign-one-struct-to-another-in-c#comment2268076_2302359
  ...
  for (uint8_t index = 0; index < MAX_BEAMERS_PER_SENSOR; ++index) {
    ...
    // for "arena" platform its impossible to infere beamer id from
    // beamer_data_array index
