 ...
 * class so that it can serve as a drop in replacement to a templated driver
 * library
 * for communicating with openBCI projects
 *
 * The implementation currently relies on the boost library, although future
 * versions may switch this
 * dependency to POCO.
 *
...
class TimeoutSerial : private boost::noncopyable,
                      public boost::enable_shared_from_this<TimeoutSerial> {
public:
  TimeoutSerial();
  ...
  TimeoutSerial(const std::string &devname, unsigned int baud_rate,
                boost::asio::boost::asio::serial_port_base::parity(
